...
function paginar($productos, $total, $pagina)
{
    //Determinar el total de paginas que genera
    //la cantidad total de productos
    $paginas = ceil(count($productos) / $total);
    ...
    echo '<table width="700" cellspacing="5" cellpadding="5">';
    echo '<tr>';
    //Obtener los N registros
    $paginado = array_slice($productos, $inicio, $final);
    ...
    if ($pagina > 1){
        echo "<a href=\"ejercicio13.php?pagina=".($pagina-1)."\">Página Anterior</a>&nbsp";
    }
    for ($i = 1; $i <= $paginas; $i++) {
        if ($i == $pagina){
            echo "<strong>$i</strong>&nbsp;";
        }else{
            echo "<a href=\"ejercicio13.php?pagina=$i\">$i</a>&nbsp;&nbsp;";
        }
    }
    if ($pagina < $paginas){
        echo "<a href=\"ejercicio13.php?pagina=".($pagina + 1)."\">Página Siguiente</a>";
    }
    echo '</td></tr>';
    echo '</table>';
    return;
